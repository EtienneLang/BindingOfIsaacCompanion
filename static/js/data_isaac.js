
const dicoTetePerso = {
        "HeadIsaac": "/static/images/CharactersHeads/HeadIsaac.png",
        "HeadMagdalene": "/static/images/CharactersHeads/HeadMagdalene.png",
        "HeadCain": "/static/images/CharactersHeads/HeadCain.png",
        "HeadJudas": "/static/images/CharactersHeads/HeadJudas.png",
        "HeadBlueBaby": "/static/images/CharactersHeads/HeadBlueBaby.png",
        "HeadEve": "/static/images/CharactersHeads/HeadEve.png",
        "HeadSamson": "/static/images/CharactersHeads/HeadSamson.png",
        "HeadAzazel": "/static/images/CharactersHeads/HeadAzazel.png",
        "HeadLazarus": "/static/images/CharactersHeads/HeadLazarus.png",
        "HeadEden": "/static/images/CharactersHeads/HeadEden.png",
        "HeadLost": "/static/images/CharactersHeads/HeadLost.png",
        "HeadLilith": "/static/images/CharactersHeads/HeadLilith.png",
        "HeadKeeper": "/static/images/CharactersHeads/HeadKeeper.png",
        "HeadApollyon": "/static/images/CharactersHeads/HeadApollyon.png",
        "HeadTheForgotten": "/static/images/CharactersHeads/HeadForgotten.png",
        "headBethany": "/static/images/CharactersHeads/HeadBethany.png",
        "HeadJacob": "/static/images/CharactersHeads/HeadJacob.png",
        "HeadTaintedIsaac": "/static/images/CharactersHeads/HeadTaintedIsaac.png",
        "HeadTaintedMagdalene": "/static/images/CharactersHeads/HeadTaintedMagdalene.png",
        "HeadTaintedCain": "/static/images/CharactersHeads/HeadTaintedCain.png",
        "HeadTaintedJudas": "/static/images/CharactersHeads/HeadTaintedJudas.png",
        "HeadTaintedBlueBaby": "/static/images/CharactersHeads/HeadTaintedBlueBaby.png",
        "HeadTaintedEve": "/static/images/CharactersHeads/HeadTaintedEve.png",
        "HeadTaintedSamson": "/static/images/CharactersHeads/HeadTaintedSamson.png",
        "HeadTaintedAzazel": "/static/images/CharactersHeads/HeadTaintedAzazel.png",
        "HeadTaintedLazarus": "/static/images/CharactersHeads/HeadTaintedLazarus.png",
        "HeadTaintedEden": "/static/images/CharactersHeads/HeadTaintedEden.png",
        "HeadTaintedLost": "/static/images/CharactersHeads/HeadTaintedLost.png",
        "HeadTaintedLilith": "/static/images/CharactersHeads/HeadTaintedLilith.png",
        "HeadTaintedKeeper": "/static/images/CharactersHeads/HeadTaintedKeeper.png",
        "HeadTaintedApollyon": "/static/images/CharactersHeads/HeadTaintedApollyon.png",
        "HeadTaintedForgotten": "/static/images/CharactersHeads/HeadTaintedForgotten.png",
        "HeadTaintedBethany": "/static/images/CharactersHeads/HeadTaintedBethany.png",
        "HeadTaintedJacob": "/static/images/CharactersHeads/HeadTaintedJacob.png",
}

const isaacAchievements = {
        "Satan" : "43",
        "BlueBaby" : "49",
        "BossRush" : "70",
        "Isaac" : "107",
        "TheLamb" : "149",
        "Heart" : "167",
        "Hush" : "179",
        "Greedier" : "296",
        "MegaSatan" : "205",
        "Delirium" : "281",
        "Mother" : "440",
        "Beast" : "441",
        "AllAchievements" : "253"
}
const magdaleneAchievements = {
        "Satan" : "45",
        "BlueBaby" : "50",
        "BossRush" : "109",
        "Isaac" : "20",
        "TheLamb" : "71",
        "Heart" : "168",
        "Hush" : "180",
        "Greedier" : "297",
        "MegaSatan" : "206",
        "Delirium" : "283",
        "Mother" : "442",
        "Beast" : "443",
        "AllAchievements" : "254"
}
const cainAchievements = {
        "Satan" : "46",
        "BlueBaby" : "75",
        "BossRush" : "110",
        "Isaac" : "21",
        "TheLamb" : "51",
        "Heart" : "171",
        "Hush" : "181",
        "Greedier" : "298",
        "MegaSatan" : "207",
        "Delirium" : "284",
        "Mother" : "444",
        "Beast" : "445",
        "AllAchievements" : "261"
}
const judasAchievements = {
        "Satan" : "72",
        "BlueBaby" : "50",
        "BossRush" : "108",
        "Isaac" : "107",
        "TheLamb" : "52",
        "Heart" : "168",
        "Hush" : "182",
        "Greedier" : "299",
        "MegaSatan" : "208",
        "Delirium" : "285",
        "Mother" : "447",
        "Beast" : "448",
        "AllAchievements" : "263"
}
const blueBabyAchievements = {
        "Satan" : "48",
        "BlueBaby" : "113",
        "BossRush" : "114",
        "Isaac" : "107",
        "TheLamb" : "73",
        "Heart" : "168",
        "Hush" : "183",
        "Greedier" : "300",
        "MegaSatan" : "209",
        "Delirium" : "286",
        "Mother" : "448",
        "Beast" : "449",
        "AllAchievements" : "252"
}
const eveAchievements = {
        "Satan" : "44",
        "BlueBaby" : "53",
        "BossRush" : "112",
        "Isaac" : "76",
        "TheLamb" : "111",
        "Heart" : "168",
        "Hush" : "186",
        "Greedier" : "302",
        "MegaSatan" : "210",
        "Delirium" : "288",
        "Mother" : "450",
        "Beast" : "451",
        "AllAchievements" : "255"
}
const samsonAchievements = {
        "Satan" : "56",
        "BlueBaby" : "53",
        "BossRush" : "115",
        "Isaac" : "54",
        "TheLamb" : "74",
        "Heart" : "168",
        "Hush" : "185",
        "Greedier" : "301",
        "MegaSatan" : "211",
        "Delirium" : "287",
        "Mother" : "452",
        "Beast" : "453",
        "AllAchievements" : "262"
}
const azazelAchievements = {
        "Satan" : "127",
        "BlueBaby" : "128",
        "BossRush" : "9",
        "Isaac" : "126",
        "TheLamb" : "47",
        "Heart" : "168",
        "Hush" : "186",
        "Greedier" : "304",
        "MegaSatan" : "212",
        "Delirium" : "290",
        "Mother" : "452",
        "Beast" : "453",
        "AllAchievements" : "259"
}
const edenAchievements = {
        "Satan" : "122",
        "BlueBaby" : "123",
        "BossRush" : "125",
        "Isaac" : "121",
        "TheLamb" : "124",
        "Heart" : "168",
        "Hush" : "188",
        "Greedier" : "303",
        "MegaSatan" : "214",
        "Delirium" : "289",
        "Mother" : "458",
        "Beast" : "459",
        "AllAchievements" : "256"
}
const lostAchievements = {
        "Satan" : "130",
        "BlueBaby" : "131",
        "BossRush" : "133",
        "Isaac" : "129",
        "TheLamb" : "132",
        "Heart" : "168",
        "Hush" : "189",
        "Greedier" : "307",
        "MegaSatan" : "215",
        "Delirium" : "293",
        "Mother" : "460",
        "Beast" : "461",
        "AllAchievements" : "256"
}
const lazarusAchievements = {
        "Satan" : "117",
        "BlueBaby" : "118",
        "BossRush" : "105",
        "Isaac" : "116",
        "TheLamb" : "119",
        "Heart" : "168",
        "Hush" : "187",
        "Greedier" : "305",
        "MegaSatan" : "213",
        "Delirium" : "291",
        "Mother" : "456",
        "Beast" : "457",
        "AllAchievements" : "257"
}
const lilithAchievements = {
        "Satan" : "220",
        "BlueBaby" : "219",
        "BossRush" : "222",
        "Isaac" : "218",
        "TheLamb" : "221",
        "Heart" : "168",
        "Hush" : "190",
        "Greedier" : "306",
        "MegaSatan" : "216",
        "Delirium" : "292",
        "Mother" : "462",
        "Beast" : "463",
        "AllAchievements" : "260"
}
const keeperAchievements = {
        "Satan" : "237",
        "BlueBaby" : "238",
        "BossRush" : "240",
        "Isaac" : "236",
        "TheLamb" : "239",
        "Heart" : "168",
        "Hush" : "191",
        "Greedier" : "308",
        "MegaSatan" : "217",
        "Delirium" : "294",
        "Mother" : "464",
        "Beast" : "465",
        "AllAchievements" : "264"
}
const apollyonAchievements = {
        "Satan" : "311",
        "BlueBaby" : "312",
        "BossRush" : "314",
        "Isaac" : "310",
        "TheLamb" : "313",
        "Heart" : "318",
        "Hush" : "315",
        "Greedier" : "309",
        "MegaSatan" : "317",
        "Delirium" : "295",
        "Mother" : "466",
        "Beast" : "467",
        "AllAchievements" : "319"
}
const forgottenAchievements = {
        "Satan" : "394",
        "BlueBaby" : "395",
        "BossRush" : "397",
        "Isaac" : "393",
        "TheLamb" : "396",
        "Heart" : "392",
        "Hush" : "398",
        "Greedier" : "400",
        "MegaSatan" : "403",
        "Delirium" : "401",
        "Mother" : "468",
        "Beast" : "469",
        "AllAchievements" : "402"
}
const bethanyAchievements = {
        "Satan" : "418",
        "BlueBaby" : "419",
        "BossRush" : "421",
        "Isaac" : "417",
        "TheLamb" : "420",
        "Heart" : "416",
        "Hush" : "423",
        "Greedier" : "424",
        "MegaSatan" : "427",
        "Delirium" : "425",
        "Mother" : "470",
        "Beast" : "471",
        "AllAchievements" : "426"
}
const jacobAchievements = {
        "Satan" : "430",
        "BlueBaby" : "431",
        "BossRush" : "433",
        "Isaac" : "429",
        "TheLamb" : "432",
        "Heart" : "428",
        "Hush" : "435",
        "Greedier" : "436",
        "MegaSatan" : "439",
        "Delirium" : "437",
        "Mother" : "472",
        "Beast" : "473",
        "AllAchievements" : "438"
}
const taintedIsaacAchievements = {
    "Satan" : "548",
    "BlueBaby" : "548",
    "BossRush" : "618",
    "Isaac" : "548",
    "TheLamb" : "548",
    "Heart" : "167", //
    "Hush" : "618",
    "Greedier" : "541",
    "MegaSatan" : "601",
    "Delirium" : "584",
    "Mother" : "549",
    "Beast" : "491",
    "AllAchievements" : "253" //
}
const taintedMagdaleneAchievements = {
    "Satan" : "550",
    "BlueBaby" : "550",
    "BossRush" : "619",
    "Isaac" : "550",
    "TheLamb" : "550",
    "Heart" : "168", //
    "Hush" : "619",
    "Greedier" : "530",
    "MegaSatan" : "602",
    "Delirium" : "585",
    "Mother" : "551",
    "Beast" : "492",
    "AllAchievements" : "254" //
}
const taintedCainAchievements = {
    "Satan" : "552",
    "BlueBaby" : "552",
    "BossRush" : "620",
    "Isaac" : "552",
    "TheLamb" : "552",
    "Heart" : "171", //
    "Hush" : "620",
    "Greedier" : "534",
    "MegaSatan" : "603",
    "Delirium" : "586",
    "Mother" : "553",
    "Beast" : "493",
    "AllAchievements" : "261" //
}
const taintedJudasAchievements = {
    "Satan" : "554",
    "BlueBaby" : "554",
    "BossRush" : "621",
    "Isaac" : "554",
    "TheLamb" : "554",
    "Heart" : "171", //
    "Hush" : "621",
    "Greedier" : "525",
    "MegaSatan" : "604",
    "Delirium" : "587",
    "Mother" : "555",
    "Beast" : "494",
    "AllAchievements" : "261" //
}
const taintedBlueBabyAchievements = {
    "Satan" : "556",
    "BlueBaby" : "556",
    "BossRush" : "622",
    "Isaac" : "556",
    "TheLamb" : "556",
    "Heart" : "171", //
    "Hush" : "622",
    "Greedier" : "528",
    "MegaSatan" : "605",
    "Delirium" : "588",
    "Mother" : "557",
    "Beast" : "495",
    "AllAchievements" : "261" //
}
const taintedEveAchievements = {
    "Satan" : "558",
    "BlueBaby" : "558",
    "BossRush" : "623",
    "Isaac" : "558",
    "TheLamb" : "558",
    "Heart" : "171", //
    "Hush" : "623",
    "Greedier" : "527",
    "MegaSatan" : "606",
    "Delirium" : "589",
    "Mother" : "559",
    "Beast" : "496",
    "AllAchievements" : "261" //
}
const taintedSamsonAchievements = {
    "Satan" : "560",
    "BlueBaby" : "560",
    "BossRush" : "624",
    "Isaac" : "560",
    "TheLamb" : "560",
    "Heart" : "171", //
    "Hush" : "624",
    "Greedier" : "535",
    "MegaSatan" : "607",
    "Delirium" : "590",
    "Mother" : "561",
    "Beast" : "497",
    "AllAchievements" : "261" //
}
const taintedAzazelAchievements = {
    "Satan" : "562",
    "BlueBaby" : "562",
    "BossRush" : "625",
    "Isaac" : "562",
    "TheLamb" : "562",
    "Heart" : "171", //
    "Hush" : "625",
    "Greedier" : "539",
    "MegaSatan" : "608",
    "Delirium" : "591",
    "Mother" : "563",
    "Beast" : "498",
    "AllAchievements" : "261" //
}
const taintedLazarusAchievements = {
    "Satan" : "564",
    "BlueBaby" : "564",
    "BossRush" : "626",
    "Isaac" : "564",
    "TheLamb" : "564",
    "Heart" : "171", //
    "Hush" : "626",
    "Greedier" : "543",
    "MegaSatan" : "609",
    "Delirium" : "592",
    "Mother" : "565",
    "Beast" : "499",
    "AllAchievements" : "261" //
}
const taintedEdenAchievements = {
    "Satan" : "566",
    "BlueBaby" : "566",
    "BossRush" : "627",
    "Isaac" : "566",
    "TheLamb" : "566",
    "Heart" : "171", //
    "Hush" : "627",
    "Greedier" : "544",
    "MegaSatan" : "610",
    "Delirium" : "593",
    "Mother" : "567",
    "Beast" : "500",
    "AllAchievements" : "261" //
}
const taintedLostAchievements = {
    "Satan" : "568",
    "BlueBaby" : "568",
    "BossRush" : "628",
    "Isaac" : "568",
    "TheLamb" : "568",
    "Heart" : "171", //
    "Hush" : "628",
    "Greedier" : "524",
    "MegaSatan" : "611",
    "Delirium" : "594",
    "Mother" : "569",
    "Beast" : "501",
    "AllAchievements" : "261" //
}
const taintedLilithAchievements = {
    "Satan" : "570",
    "BlueBaby" : "570",
    "BossRush" : "629",
    "Isaac" : "570",
    "TheLamb" : "570",
    "Heart" : "171", //
    "Hush" : "629",
    "Greedier" : "526",
    "MegaSatan" : "612",
    "Delirium" : "595",
    "Mother" : "571",
    "Beast" : "502",
    "AllAchievements" : "261" //
}
const taintedKeeperAchievements = {
    "Satan" : "572",
    "BlueBaby" : "572",
    "BossRush" : "630",
    "Isaac" : "572",
    "TheLamb" : "572",
    "Heart" : "171", //
    "Hush" : "630",
    "Greedier" : "536",
    "MegaSatan" : "613",
    "Delirium" : "596",
    "Mother" : "573",
    "Beast" : "503",
    "AllAchievements" : "261" //
}
const taintedApollyonAchievements = {
    "Satan" : "574",
    "BlueBaby" : "574",
    "BossRush" : "631",
    "Isaac" : "574",
    "TheLamb" : "574",
    "Heart" : "171", //
    "Hush" : "631",
    "Greedier" : "540",
    "MegaSatan" : "614",
    "Delirium" : "597",
    "Mother" : "575",
    "Beast" : "504",
    "AllAchievements" : "261" //
}
const taintedForgottenAchievements = {
    "Satan" : "576",
    "BlueBaby" : "576",
    "BossRush" : "632",
    "Isaac" : "576",
    "TheLamb" : "576",
    "Heart" : "171", //
    "Hush" : "632",
    "Greedier" : "537",
    "MegaSatan" : "615",
    "Delirium" : "598",
    "Mother" : "577",
    "Beast" : "505",
    "AllAchievements" : "261" //
}
const taintedBethanyAchievements = {
    "Satan" : "578",
    "BlueBaby" : "578",
    "BossRush" : "633",
    "Isaac" : "578",
    "TheLamb" : "578",
    "Heart" : "171", //
    "Hush" : "633",
    "Greedier" : "529",
    "MegaSatan" : "616",
    "Delirium" : "599",
    "Mother" : "579",
    "Beast" : "506",
    "AllAchievements" : "261" //
}
const taintedJacobAchievements = {
    "Satan" : "580",
    "BlueBaby" : "580",
    "BossRush" : "634",
    "Isaac" : "580",
    "TheLamb" : "580",
    "Heart" : "171", //
    "Hush" : "634",
    "Greedier" : "542",
    "MegaSatan" : "617",
    "Delirium" : "600",
    "Mother" : "581",
    "Beast" : "507",
    "AllAchievements" : "261" //
}

const dataAllAchievements = {
    "isaac" :isaacAchievements,
    "magdalene" : magdaleneAchievements,
    "cain" : cainAchievements,
    "judas" : judasAchievements,
    "blueBaby" : blueBabyAchievements,
    "eve" : eveAchievements,
    "samson" : samsonAchievements,
    "azazel" : azazelAchievements,
    "lazarus" : lazarusAchievements,
    "eden" : edenAchievements,
    "lost" : lostAchievements,
    "lilith" : lilithAchievements,
    "keeper" : keeperAchievements,
    "apollyon" : apollyonAchievements,
    "forgotten" : forgottenAchievements,
    "bethany" : bethanyAchievements,
    "jacob" : jacobAchievements,
    "taintedIsaac" : taintedIsaacAchievements,
    "taintedMagdalene" : taintedMagdaleneAchievements,
    "taintedCain" : taintedCainAchievements,
    "taintedJudas" : taintedJudasAchievements,
    "taintedBlueBaby" : taintedBlueBabyAchievements,
    "taintedEve" : taintedEveAchievements,
    "taintedSamson" : taintedSamsonAchievements,
    "taintedAzazel" : taintedAzazelAchievements,
    "taintedLazarus" : taintedLazarusAchievements,
    "taintedEden" : taintedEdenAchievements,
    "taintedLost" : taintedLostAchievements,
    "taintedLilith" : taintedLilithAchievements,
    "taintedKeeper" : taintedKeeperAchievements,
    "taintedApollyon" : taintedApollyonAchievements,
    "taintedForgotten" : taintedForgottenAchievements,
    "taintedBethany" : taintedBethanyAchievements,
    "taintedJacob" : taintedJacobAchievements
}

const dataTextGuide= {
    //Structure du dictionnaire : [Onglet du guide Cliqué][Numéro d'étape pour la chose à faire][0 = texte de l'étape, 1 = image de l'étape]
    "GuideSatan":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
        "2" : ["Defeat Mom's Heart to unlock the trapdoor leading down to Sheol. Make sure to go in the trapdoor and NOT in the beam of light", "/static/images/GuideImages/HeartBossGuide.png"],
        "3" : ["Find the boss room in Sheol and defeat Satan", "/static/images/GuideImages/SatanBossGuide.png"],
    },
    "GuideIsaac":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
        "2" : ["Defeat Mom's Heart to unlock the trapdoor leading down to Cathedral. Make sure to go in the beam of light and NOT in the trapdoor", "/static/images/GuideImages/HeartBossGuide.png"],
        "3" : ["Find the boss room in Cathedral and defeat Isaac", ""],
    },
    "GuideBlueBaby":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
        "2" : ["Defeat Mom's Heart to unlock the trapdoor leading down to Cathedral.", "/static/images/GuideImages/HeartBossGuide.png"],
        "3" : ["After Mom's hearts, make sure to take the item at the right (The polaroid) and go in the beam of light, NOT in the trapdoor", "/static/images/GuideImages/Polaroid.png"],
        "4" : ["Find the boss room in Cathedral and defeat Isaac", "/static/images/GuideImages/IsaacBossGuide.png"],
        "5" : ["After Isaac, go in the trapdoor that appears and defeat ???", "/static/images/GuideImages/BlueBabyBossGuide.png"],
    },
    "GuideTheLamb":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
        "2" : ["Defeat Mom's Heart to unlock the trapdoor leading down to Cathedral.", "/static/images/GuideImages/HeartBossGuide.png"],
        "3" : ["After Mom's hearts, make sure to take the item at the right (The negative) and go in the trapdoor, NOT in the beam of light", "/static/images/GuideImages/Negative.png"],
        "4" : ["Find the boss room in Sheol and defeat Satan", "/static/images/GuideImages/SatanBossGuide.png"],
        "5" : ["After Satan, go in the chest that appears and defeat find the boss room to defeat The Lamb", "/static/images/GuideImages/TheLambBossGuide.png"],
    },
    "GuideHush":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
        "2" : ["Defeat Mom's Heart in less than 30 minutes", "/static/images/GuideImages/HeartBossGuide.png"],
        "3" : ["Go in the hole that has appeared on the wall.", ""],
        "4" : ["Defeat Hush", "/static/images/GuideImages/HushBossGuide.png"],
    },
    "GuideBossRush":{
        "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom under 20 minutes.", "/static/images/GuideImages/MomBossGuide.png"],
        "3" : ["Go in the hole that has appeared on the wall.", ""],
        "4" : ["Take an item. Directly after that the boss rush will start. Defeat the 15 rounds of 2 bosses",""],
    },
    "GuideMegaSatan":{
        "1" : ["To do a mega Satan run, you need to skip all devil rooms", ""],
        "2" : ["You also need to avoid the maximum amount of damage you take so your angel deals chances stays up and keep 2 bombs minimum for the angels", ""],
        "3" : ["Once you enter an angel room, take the item and blow up the angel. Defeat the boss and take the key item", ""],
        "4" : ["After that, do the same thing on an other floor", ""],
        "5" : ["Once you have the two key pieces, do a normal run going to the chest or the dark room", ""],
        "6" : ["Once in the chest or dark room, the big door will unlock", ""],
        "7" : ["Simply go into the door and defeat Mega Satan", ""],
    },
    // "GuideDelirum" : {
    //     "1" : ["Make your way to the Depths 2 floor, where you'll find the Mom boss fight. Defeat Mom to unlock the trapdoor leading down to Utero.", "/static/images/GuideImages/MomBossGuide.png"],
    //     "2" : ["If you beat hush, the portal to the void will have 100% to spawn. So technically it's the best way to go. But you can also drop down and ", "/static/images/GuideImages/HeartBossGuide.png"],
    //     "3" : ["Go in the hole that has appeared on the wall.", ""],
    //     "4" : ["Defeat Hush", "/static/images/GuideImages/HushBossGuide.png"],
    // },
    "GuideMother" : {
        "1" : ["Go to DownPour in the first or second floor", "/static/images/GuideImages/door_downpourDoor.png"],
        "2" : ["On DownPour's second floor, go in the mirror dimension and grab the first knife piece in the item room.", "/static/images/GuideImages/MirrorDoor.png"],
        "3" : ["Go to Ashpit/The Mines after the DownPour second floor's boss", "/static/images/GuideImages/MinesDoor.png"],
        "4" : ["On AshPit/Mines second floor, click the three buttons and go take the second knife piece.(Remember you will have to lose two hearts to enter mausoleum)", "/static/images/GuideImages/MinesBtn.png"],
        "5" : ["Go to Mausoleum after AshPit/The Mines second floor's boss", "/static/images/GuideImages/MusauleumDoor.png"],
        "6" : ["Beat mom on the second floor of mausoleum, DON'T DROP DOWN. Break the flesh door with the knife.", "/static/images/GuideImages/FleshDoor.png"],
        "7" : ["Complete the corpse and beat mother", "/static/images/GuideImages/MotherBossGuide.png"]
    },
    "GuideBeast" : {
        "1" : ["Drop one trinket in any treasure room or boss room during the floors before mom.", "/static/images/GuideImages/TreasureRoom.png"],
        "2" : ["Do a normal run until you arrive to the Depths 2", ""],
        "3" : ["Search for a skull marked with an X and destroy it. Take the card and keep it until the boss fight.", "/static/images/GuideImages/XSkull.png"],
        "4" : ["Beat mom and DO NOT GO DOWN. Take the Polaroid or the Negative and use the fool card.", "/static/images/GuideImages/Negative.png"],
        "5" : ["Go to each treasure rooms through the ascension to find your cracked key (The trinket you dropped has transformed into a cracked key)", ""],
        "6" : ["Complete the ascension and go Home", ""],
        "7" : ["Use your cracked key on the red room at the left wall under mom's bedroom.", ""],
        "8" : ["Before you go to sleep, take the item in the chest bellow mother's bed and sleep in her bed.", "/static/images/GuideImages/momChest.png"],
        "9" : ["Beat Dogma, all the mega sins and The Beast","/static/images/GuideImages/BeastBossGuide.png"]
    }
}

const dataMainProgression = {
    //Structure du dictionnaire [Id de l'achievement][0 = Description de l'unlock, 1 = Nom de ce que ça débloque]
    "4" : ["Defeat Mom", "The Womb"],
    "150" : ["Defeat Mom's Heart 2 times", "Rubber Cement"],
    "234" : ["Defeat Mom's Heart 10 times", "Blue Womb"],
    "34" : ["Defeat Mom's Heart 11 times", "It Lives!"],
    "57" : ["Defeat Isaac 5 times", "The Polaroid"],
    "78" : ["Defeat Satan 5 times", "The Negative"],
    "320" : ["Defeat Hush", "New Area (The void for Delirium)"],
    "406" : ["Collect 3 items with the 'stars' tag in one run", "The Planetarium"],
    "407" : ["Beat Hush 3 times", "A Secret Ending (Path to mother)"],
    "635" : ["Defeat Mother", "A strange door (Path to The Beast)"]
}

const dataGoodItemsProgression = {
    //Structure du dictionnaire [Id de l'achievement][0 = Description de l'unlock, 1 = Nom de ce que ça débloque]
    "11" : ["Defeat Mom's Heart 5 times", "Dr. Fetus"],
    "29" : ["Defeat Isaac with BlueBaby", "The D6"],
    "43" : ["Defeat Satan with Isaac", "Mom's Knife"],
    "52" : ["Defeat The Lamb with Judas", "Curved Horn"],
    "62" : ["Complete The Family man challenge", "Epic Fetus"],
    "103": ["Complete Waka Waka challenge", "Death's Touch"],
    "112": ["Complete Boss Rush with Eve", "Eve's Mascara"],
    "138" : ["Donate 999 coins to the donation machine", "Stop Watch"],
    "140" : ["Defeat Mom's Heart 6 times", "Ipecac"],
    "156" : ["Earn all completion marks with the lost", "Godhead"],
    "190" : ["Defeat Hush with Lilith", "Incubus"],
    "198" : ["Defeat Ultra Greed with Samson", "Lusty Blood"],
    "282" : ["Defeat Delirium with Isaac", "D Infinity"],
    "289" : ["Defeat Delirium with Eden", "Eden's soul"],
    "291" : ["Defeat Delirium with Lazarus", "Compound Fracture"],
    "318" : ["Defeat Mom's heart with Apollyon in hard mode", "Smelter"],
    "377" : ["Acquire Blood Clot 10 times", "Haemolacria"],
    "379" : ["Enter 6 shops in one run", "Schoolbag"],
    "491" : ["Defeat The Beast with Tainted Isaac", "Glitched Crown"],
    "396" : ["Defeat The Lamb with The Forgotten", "Brittle Bones"],
    "415" : ["Open the chest in mom's room", "Red Key"],
    "431" : ["Defeat BlueBaby with Jacob and Esau", "Birthright"],
    "432" : ["Defeat The Lamb with Jacob and Esau", "Damocles"],
    "433" : ["Defeat The Boss Rush with Jacob and Esau", "Rock Bottom"],
    "436" : ["Defeat Ultra Greedier with Jacob and Esau", "Genesis"],
    "448" : ["Defeat Mother with BlueBaby", "Eternal D6"],
    "463" : ["Defeat The Beast with Lilith", "C Section"],
    "464" : ["Defeat Mother with Keeper", "Keeper's Sack"],
    "470" : ["Defeat Mother with Bethany", "Revelation"],
    "501" : ["Defeat The Beast with Tainted Lost", "Sacred Orb"],
    "502" : ["Defeat The Beast with Tainted Lilith", "Twisted Pair"],
    "520" : ["Complete Isaac's Awakening challenge (#39)", "Spirit Sword"],
}

const dataCharactersProgression = {
    //Structure du dictionnaire [Id de l'achievement][0 = Description de l'unlock, 1 = Nom de ce que ça débloque]
    "1" : ["Have 7 or more Red Heart Containers at one time", "Magdalene"],
    "2" : ["Hold 55 Coins at one time", "Cain"],
    "3" : ["Defeat Satan", "Judas"],
    "32" : ["Defeat Mom's Heart 10 times", "Blue Baby"],
    "42" : ["Don't pick up any Hearts for 2 floors in a row", "Eve"],
    "67" : ["Complete 2 floors in a row without taking damage", "Samson"],
    "79" : ["Make 3 Deals with the Devil in one run", "Azazel"],
    "80" : ["Have 4 or more Soul Hearts at one time", "Lazarus"],
    "81" : ["Beat all Utero", "Eden"],
    "82" : ["Die in a Sacrifice Room while holding Missing Poster", "The Lost"],
    "199" : ["Defeat Ultra Greed with Azazel", "Lilith"],
    "251" : ["Donate 1000 coins to the Greed Machine", "Keeper"],
    "340" : ["Defeat Mega Satan", "Apollyon"],
    "390" : ["Guide Comming Soon...", "The Forgotten"],
    "404" : ["Beat Hard mode with Lazarus without losing a life", "Bethany"],
    "405" : ["Beat Mother", "Jacob and Esau"],
    "474" : ["Use the red key to open the closet in Home with Isaac", "Tainted Isaac"],
    "475" : ["Use the red key to open the closet in Home with Magdalene", "Tainted Magdalene"],
    "476" : ["Use the red key to open the closet in Home with Cain", "Tainted Cain"],
    "477" : ["Use the red key to open the closet in Home with Judas", "Tainted Judas"],
    "478" : ["Use the red key to open the closet in Home with Blue Baby", "Tainted Blue Baby"],
    "479" : ["Use the red key to open the closet in Home with Eve", "Tainted Eve"],
    "480" : ["Use the red key to open the closet in Home with Samson", "Tainted Samson"],
    "481" : ["Use the red key to open the closet in Home with Azazel", "Tainted Azazel"],
    "482" : ["Use the red key to open the closet in Home with Lazarus", "Tainted Lazarus"],
    "483" : ["Use the red key to open the closet in Home with Eden", "Tainted Eden"],
    "484" : ["Use the red key to open the closet in Home with The Lost", "Tainted Lost"],
    "485" : ["Use the red key to open the closet in Home with Lilith", "Tainted Lilith"],
    "486" : ["Use the red key to open the closet in Home with Keeper", "Tainted Keeper"],
    "487" : ["Use the red key to open the closet in Home with Apollyon", "Tainted Apollyon"],
    "488" : ["Use the red key to open the closet in Home with The Forgotten", "Tainted Forgotten"],
    "489" : ["Use the red key to open the closet in Home with Bethany", "Tainted Bethany"],
    "490" : ["Use the red key to open the closet in Home with Jacob and Esau", "Tainted Jacob"],
}